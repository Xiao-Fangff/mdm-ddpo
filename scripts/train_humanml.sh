...
STEP_REWARD_CALIBRATION_PATH="${MDM_DDPO_STEP_REWARD_CALIBRATION_PATH:-${STEP_REWARD_CALIBRATION_PATH:-${PROJECT_ROOT}/step_reward_k16_calibration.json}}"
FIXED_STEP_EVAL_POOL_PATH="${MDM_DDPO_FIXED_STEP_EVAL_POOL_PATH:-${FIXED_STEP_EVAL_POOL_PATH:-${PROJECT_ROOT}/artifacts/step_val_fixed_eval_pool_k16.pt}}"
...
ROLLOUT_BATCH_SIZE="${MDM_DDPO_ROLLOUT_BATCH_SIZE:-32}"
ROLLOUT_BATCHES_PER_EPOCH="${MDM_DDPO_ROLLOUT_BATCHES_PER_EPOCH:-4}"
...
case "${DDPO_ENABLE_STEP_REWARD,,}" in
  1|true|yes|on)
    # K_step=16 with a 25% motion allocation needs at least 64 motions:
    # 12 HumanML prompts x 4 plus 1 step prompt x 16.
    ROLLOUT_BATCH_SIZE="${MDM_DDPO_STEP_ROLLOUT_BATCH_SIZE:-64}"
    ROLLOUT_BATCHES_PER_EPOCH="${MDM_DDPO_STEP_ROLLOUT_BATCHES_PER_EPOCH:-2}"
    STEP_ARGS=(
      --enable-step-reward
...
exec "${PYTHON}" "${PROJECT_ROOT}/train_ddpo.py" \
  ...
  --rollout-batches-per-epoch "${ROLLOUT_BATCHES_PER_EPOCH}" \
