...
- 记录 `log_ratio`、ratio dispersion、LoRA/count/trainable parameter norm、各参数组
  update norm 和非有限梯度。
...
- checkpoint prediction type 使用 `auto/x_start/epsilon` 显式解析；`auto`
  优先读取配套 `args.json` 的 `prediction_type` 或 `predict_epsilon`，避免把
  epsilon checkpoint 悄然按 x0 policy 计算 DDIM transition 与 log-prob。
...
对旧 x0 MDM 完成的 24 conditions × 2 noises × 6 targets（每个 policy 288 motions）
正式 probe 表明：hard target-count Spearman 为 `0.4540`，target standardized effect
为 `0.5247`，length effect 为 `0.1810`；projected `embed_text` 的最小 RMS 距离为
`0.02306`。该 x0 模型确实区分并使用数字 token，主要问题是 count response 在约
4 步处饱和，尤其 target 2 exact 为 0。新 epsilon MDM 的数字响应更弱，不能沿用
这个结论。
完整分析见 `reports/counterfactual_number_probe_analysis.md`。

### MDM prediction type 与 DDPO timestep 诊断

`tools/diagnose_ddpo_mdm_compatibility.py` 会固定 prompt、length、初始 latent 和每个
DDIM transition noise，审计完整轨迹 old/new log-prob，并按 timestep bucket 记录
advantage-weighted gradient、ratio/clip，以及重复 overfit 同一轨迹时的 reward 响应。

当前 50-step cosine MDM 的实测结果表明，x0 policy 的梯度高度集中在反向扩散末端：
`t=1--2` bucket 的梯度约为 `t=31--49` 的 98.6 倍；首次 update 后两者 clip
fraction 分别为 90.6% 和 0%。正确加载的 50-step epsilon checkpoint 将各 bucket
梯度缩小到约 3.7 倍范围，说明 prediction type 确实是 DDPO credit imbalance 的一个
根因。

但现有 epsilon checkpoints 的零 LoRA HumanML baseline 明显低于 x0：50-step
retrieval/M2M 为 `-0.082/0.051`，200-step 为 `0.109/0.169`，1000-step 为
`0.060/0.131`，而 x0 对照为 `0.641/0.321`。因此这些 checkpoint 不能直接用于正式
DDPO；必须先在标准 HumanML evaluation 和同一 fixed pool 上达到可比 baseline。
完整数值与解释见 `reports/ddpo_mdm_prediction_type_comparison.md`。

新训练的 `epsilon + linear-50 + Min-SNR-5 + x_start/velocity auxiliary`
checkpoint 已完成同池复核。它把有效 epsilon transition sensitivity 压到约
`4.68x`，并通过完整 log-prob 审计；retrieval/M2M 组内排序信号也明显强于原
x0 模型。但 counterfactual target-count Spearman 仅 `0.096`，length effect 为
`0.427`，synthetic K8 hard MAE 为 `2.72`，且脚踝高频比约 `0.325`。6-epoch
隔离 DDPO 只改善 soft 指标，没有任何 hard acceptance point，因此不建议直接
扩展到 30/100 epochs。完整结果见
`reports/epsilon_linear50_minsnr5_150k_evaluation.md`；可复现实验入口为
`scripts/train_epsilon_linear50_minsnr5_step_k8_short.sh`。

### Explicit count conditioning 与 native diffusion SFT

上述 epsilon MDM 的 counterfactual hard target-count Spearman 只有 `0.0959`，因此
已启用显式 count conditioning，而不是继续单独增大 DDPO 权重：

```text
count id 0..6 : 具体步数条件
count id 7    : 普通 HumanML / no-count
```

`count_embedding -> zero-initialized projection` 加到 MDM 的 time/text conditioning；
零初始化保证安装前后输出逐元素一致，CFG unconditional 分支强制屏蔽 count，普通
HumanML 的 no-count 路径也严格为零。checkpoint、calibration、probe、export 和
DDPO 都保存并校验 count structure、base diffusion metadata 与 policy SHA-256，避免
把 epsilon/linear-50 或错误的 LoRA/count policy 混用。

短期 SFT 使用 MDM 原生 `diffusion.training_losses`，严格保留该 checkpoint 的
`epsilon + linear-50 + FIXED_SMALL + Min-SNR-5 + xstart(1.0) + velocity(0.1)` 配置。
step real motions 在 target 1--6 和共享 length bins 内均衡：共 618 条，每个 target
103 条，共同长度范围 52--168，各 target 平均长度相差不到 1.1 帧。每次 update 混合
32 HumanML + 32 step motions；anti-jitter loss 对 HumanML 263-D 表示中的 foot/ankle
acceleration target 计算，并在首次 update 自动标定到 native gradient 的 10%。默认
训练 3 epochs（57 updates）：

```bash
cd /home/zhiwei/projects/mdm-ddpo
CUDA_VISIBLE_DEVICES=7 DEVICE=cuda:0 \
OUTPUT_DIR=$PWD/outputs/epsilon_count_sft_seed42 \
bash scripts/train_count_conditioning_sft.sh --seed 42
```

选择 SFT checkpoint 后必须从该**精确 LoRA/count policy**重新生成两份 calibration；
工具会保存并在 DDPO 启动时校验 policy SHA-256：
...
SFT_CKPT=$PWD/outputs/epsilon_count_sft_seed42/checkpoint_000002.pt

CUDA_VISIBLE_DEVICES=7 python tools/calibrate_reward_stats.py \
  --output outputs/epsilon_count_sft_seed42/reward_calibration_epoch2_full.json \
  --pool-path artifacts/reward_calibration_pool.pt \
  --samples-output outputs/epsilon_count_sft_seed42/reward_calibration_epoch2_full_samples.pt \
  --prompts 1024 --samples-per-prompt 4 --batch-size 64 --sample-steps 50 \
  --model-path "$MODEL_DIR/model000150000.pt" \
  --model-args-path "$MODEL_DIR/args.json" \
  --policy-checkpoint "$SFT_CKPT" --prediction-type epsilon \
  --device cuda:0 --precision bf16

CUDA_VISIBLE_DEVICES=7 python tools/calibrate_step_reward_stats.py \
  --output outputs/epsilon_count_sft_seed42/step_reward_k8_soft_huber_epoch2_full.json \
  --pool-path artifacts/step_reward_k8_soft_huber_pool.pt \
  --samples-output outputs/epsilon_count_sft_seed42/step_reward_k8_soft_huber_epoch2_full_samples.pt \
  --prompts 384 --samples-per-prompt 8 --batch-size 64 --sample-steps 50 \
  --step-targets 1,2,3,4,5,6 --step-pool-source synthetic \
  --step-detector-backend progressive --step-reward-mode soft_huber_exact \
  --model-path "$MODEL_DIR/model000150000.pt" \
  --model-args-path "$MODEL_DIR/args.json" \
  --policy-checkpoint "$SFT_CKPT" --prediction-type epsilon \
  --device cuda:0 --precision bf16
...
同池 counterfactual 选择 `checkpoint_000002.pt`：

| policy | hard target Spearman | hard MAE | exact | within-one | ankle HF |
| --- | ---: | ---: | ---: | ---: | ---: |
| 原 epsilon MDM | 0.0959 | 2.3889 | 0.1215 | 0.3646 | 0.3565 |
| count SFT epoch 2 | 0.4749 | 2.2188 | 0.1597 | 0.3993 | 0.3622 |
| count-only DDPO epoch 5 | 0.5043 | 2.1076 | 0.1632 | 0.4271 | 0.3670 |

SFT 后 HumanML fixed-val retrieval/M2M delta 为 `+0.000329/+0.000574`。标准
HumanML quick eval 中 R-precision top-1/2/3 从 `0.3750/0.5625/0.6875` 提高到
`0.3906/0.5781/0.7031`，matching 基本不变；FID 从 `3.3203` 到 `3.4205`，变化
`+0.1001`，低于既有 baseline 重复评测标准差 `0.114`，但正式结论仍需完整评测。

SFT 后的 DDPO 推荐冻结共享 LoRA，只更新 266,240 个 count 参数，避免普通 HumanML
路径漂移。下面的 6-epoch 诊断固定使用 K8、50% step motions、每 epoch 16 个均衡
step prompt groups、soft-Huber reward、`lr=1e-4` 和 `clip=1e-3`：

```bash
cd /home/zhiwei/projects/mdm-ddpo
CUDA_VISIBLE_DEVICES=7 DEVICE=cuda:0 REWARD_DEVICE=same \
OUTPUT_DIR=$PWD/outputs/epsilon_count_only_step_k8_lr1e4_clip1e3 \
MDM_DDPO_USE_SWANLAB=0 \
bash scripts/train_epsilon_count_only_step_k8_short.sh
```

fixed step val 的最终 paired delta 为 reward `+0.02589`、MAE `-0.10938`、exact
`+0.00260`、within-one `+0.03385`；HumanML retrieval/M2M delta 都严格为 0。
首次 old/new ratio audit 误差为 0，24/24 updates 有限，clip fraction 为 0，ratio
std 为 `3.6e-5--4.3e-5`；`count_update_norm` 非零而 `lora_update_norm=0`。完整三组
DDPO 对比见 `reports/epsilon_count_sft_ddpo_comparison.md`。

这些结果证明显式 count SFT 有效，也给出了一个 count-only DDPO hard acceptance
checkpoint：`best_step_acceptance.pt`。但 exact 只在最后一个 fixed validation point
转正，且 exact delta 的 bootstrap SE 为 `0.00915`，尚未满足统计显著、多点、多 seed
稳定性；counterfactual ankle HF 相对原模型增加
`0.0105`（相对 SFT 增加 `0.0048`）。因此当前 checkpoint 只作为诊断候选，不应直接
宣称已解决真实步数控制；下一步应做 seed 43/44、人工视频检查和完整 HumanML eval，
而不是直接延长到 30/100 epochs。
...
异常。新 epsilon checkpoint 已满足“文本 embedding 可区分、但生成 count 对数字
响应很弱”的触发条件，因此当前推荐流程已经转为上一节的 explicit count SFT，再以
count-only DDPO 做小步精调。
...
| 参数 | 说明 |
| --- | --- |
| `--prediction-type` | `auto`（默认）、`x_start` 或 `epsilon`；auto 从 checkpoint `args.json` 解析，旧 checkpoint 回退到 x_start |
| `--enable-count-conditioning` | 安装 0--6 + no-count 的显式 count adapter；默认关闭，零初始化时不改变 base MDM |
| `--initial-policy-path` | 从 native count SFT checkpoint 初始化 LoRA/count policy，但不恢复 DDPO optimizer/epoch |
| `--train-count-conditioning` | 是否允许 DDPO 更新 count adapter；启用 count conditioning 时默认开启 |
| `--train-lora` / `--no-train-lora` | 是否同时更新 LoRA；count-only DDPO 使用后者保护 HumanML no-count 路径 |
